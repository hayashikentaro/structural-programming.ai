...
ここでは、Functor を読むために必要な対応だけを取り出しておく。
...
| `A -> B` | ある型から別の型への関数 |
| `F<A>` | 文脈つきの型 |
| `map` | 中身の関数を文脈つき型へ持ち上げる操作 |

※ Functor の要点は、`F<A>` のような型を作れることだけではない。`A -> B` という関数に対して、`F<A> -> F<B>` を対応させられることである。さらにその対応は、恒等と合成を壊してはならない。
